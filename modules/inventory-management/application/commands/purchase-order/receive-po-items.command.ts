...
export class ReceivePOItemsCommandHandler
  implements ICommandHandler<ReceivePOItemsCommand, CommandResult<ReceivePOItemsResult>>
{
      ...
      if (command.items) {
        command.items.forEach((item, index) => {
          ...
          if (!item.receivedQty || item.receivedQty <= 0) {
            errors.push(`items[${index}].receivedQty: Received quantity must be greater than 0`);
          }
        });
      }

      if (errors.length > 0) {
        return CommandResult.failure<ReceivePOItemsResult>("Validation failed", errors);
      }
