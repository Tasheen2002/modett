...
export class CreatePickupReservationCommandHandler
  implements ICommandHandler<CreatePickupReservationCommand, CommandResult<PickupReservation>>
{
  ...
  async handle(
    ...
    try {
      ...
      if (command.expirationMinutes !== undefined && command.expirationMinutes <= 0) {
        errors.push("expirationMinutes: Expiration minutes must be greater than 0");
      }

      if (errors.length > 0) {
        return CommandResult.failure<PickupReservation>("Validation failed", errors);
      }
