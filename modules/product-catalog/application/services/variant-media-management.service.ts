...
export class VariantMediaManagementService {
  ...
  async addMediaToVariant(variantId: string, assetId: string): Promise<void> {
    ...
    const existingAssociation = await this.variantMediaRepository.findAssociation(variantIdVo, assetIdVo);
    if (existingAssociation) {
      throw new Error('Media asset is already associated with this variant');
    }
  ...
  async removeMediaFromVariant(variantId: string, assetId: string): Promise<void> {
    ...
    if (!association) {
      throw new Error('Media asset is not associated with this variant');
    }
  ...
  async getVariantMedia(variantId: string): Promise<VariantMediaSummary> {
    ...
    const mediaAssets = await Promise.all(
      variantMediaList.map(async (variantMedia) => {
        const assetIdEntity = EntityMediaAssetId.fromString(variantMedia.getAssetId().toString());
        const asset = await this.mediaAssetRepository.findById(assetIdEntity);
        return {
  ...
  async duplicateVariantMedia(sourceVariantId: string, targetVariantId: string): Promise<void> {
    ...
    await this.variantMediaRepository.duplicateVariantMedia(sourceVariantIdVo, targetVariantIdVo);
  }

  async getProductVariantMedia(productId: string, options: VariantMediaServiceQueryOptions = {}): Promise<ProductVariantMediaSummary> {
    ...
    const productVariantMediaData = await this.variantMediaRepository.getProductVariantMedia(productIdVo);
      ...
      productVariantMediaData.map(async (item) => {
        ...
        const mediaAssets = await Promise.all(
          item.media.map(async (vm) => {
            const assetIdEntity = EntityMediaAssetId.fromString(vm.getAssetId().toString());
        const asset = await this.mediaAssetRepository.findById(assetIdEntity);
            return {
  ...
  async copyProductVariantMedia(sourceProductId: string, targetProductId: string, variantMapping: Record<string, string>): Promise<void> {
    ...
    for (const [sourceVariantId, targetVariantId] of Object.entries(variantMapping)) {
      variantIdMapping.set(VariantId.fromString(sourceVariantId), VariantId.fromString(targetVariantId));
    }

    await this.variantMediaRepository.copyProductVariantMedia(sourceProductIdVo, targetProductIdVo, variantIdMapping);
  }
  ...
  async getColorVariantMedia(productId: string, color: string): Promise<ColorVariantMedia> {
    ...
    const variants = await Promise.all(
      Array.from(variantMediaMap.entries()).map(async ([variantIdStr, mediaList]) => {
  ...
  async getSizeVariantMedia(productId: string, size: string): Promise<Array<{
    size: string;
    variants: Array<{
      variantId: string;
      sku: string;
  ...
  async validateVariantMedia(variantId: string): Promise<{
    ...
    for (const vm of variantMedia) {
      ...
      const asset = await this.mediaAssetRepository.findById(assetIdEntity);
      if (!asset) {
        issues.push(`Referenced media asset ${vm.getAssetId().toString()} not found`);
      }
    }
