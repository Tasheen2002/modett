...
export class ProductMediaManagementService {
  ...
  async addMediaToProduct(
    ...
    if (existingAssociation) {
      throw new Error("Media asset is already associated with this product");
    }
  ...
  async removeMediaFromProduct(
    ...
    if (!association) {
      throw new Error("Media asset is not associated with this product");
    }
  ...
  async setProductMedia(
    ...
    for (const item of mediaData) {
      ...
      const asset = await this.mediaAssetRepository.findById(assetIdEntity);
      if (!asset) {
        throw new Error(`Media asset ${item.assetId} not found`);
      }
    }
    ...
    if (coverImages.length > 1) {
      throw new Error("Only one media asset can be set as cover image");
    }
  ...
  async getProductMediaStatistics(productId: string): Promise<{
    ...
    for (const pm of productMedia) {
      const assetIdEntity = EntityMediaAssetId.fromString(pm.getAssetId().toString());
      const asset = await this.mediaAssetRepository.findById(assetIdEntity);
      if (asset) {
