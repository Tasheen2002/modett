...
export interface IProductTagAssociationRepository {
  ...
  removeTagFromProduct(productId: ProductId, tagId: ProductTagId): Promise<void>;
  ...
  findAssociation(productId: ProductId, tagId: ProductTagId): Promise<ProductTagAssociation | null>;
  findAll(options?: ProductTagAssociationQueryOptions): Promise<ProductTagAssociation[]>;

  // Get related IDs
  getProductTags(productId: ProductId): Promise<ProductTagId[]>;
  getTaggedProducts(tagId: ProductTagId, options?: ProductTagAssociationQueryOptions): Promise<ProductId[]>;

  // Bulk operations
  setProductTags(productId: ProductId, tagIds: ProductTagId[]): Promise<void>;
  addTagToMultipleProducts(tagId: ProductTagId, productIds: ProductId[]): Promise<void>;
  addMultipleTagsToProduct(productId: ProductId, tagIds: ProductTagId[]): Promise<void>;

  // Search and filtering methods
  findProductsByTags(tagIds: ProductTagId[], matchAll?: boolean, options?: ProductTagAssociationQueryOptions): Promise<ProductId[]>;
  findProductsByTagNames(tagNames: string[], matchAll?: boolean, options?: ProductTagAssociationQueryOptions): Promise<ProductId[]>;
  findSimilarProducts(productId: ProductId, minCommonTags?: number, options?: ProductTagAssociationQueryOptions): Promise<ProductId[]>;
  ...
  getMostPopularTags(limit?: number): Promise<Array<{ tagId: ProductTagId; productCount: number }>>;
  getUntaggedProducts(options?: ProductTagAssociationQueryOptions): Promise<ProductId[]>;
  getTagUsageStats(): Promise<Array<{ tagId: ProductTagId; usageCount: number }>>;

  // Category-specific tag operations
  findTagsByCategory(categoryId: string): Promise<ProductTagId[]>;
  getProductTagsInCategory(productId: ProductId, categoryId: string): Promise<ProductTagId[]>;

  // Brand-specific tag operations
  findTagsByBrand(brand: string): Promise<ProductTagId[]>;
  getProductTagsForBrand(productId: ProductId, brand: string): Promise<ProductTagId[]>;
}
