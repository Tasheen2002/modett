...
export interface IProductMediaRepository {
  ...
  addMediaToProduct(productId: ProductId, assetId: MediaAssetId, position?: number, isCover?: boolean): Promise<ProductMediaId>;
  removeMediaFromProduct(productId: ProductId, assetId: MediaAssetId): Promise<void>;
  removeAllProductMedia(productId: ProductId): Promise<void>;
  removeAllAssetReferences(assetId: MediaAssetId): Promise<void>;

  // Query methods
  findByProductId(productId: ProductId, options?: ProductMediaQueryOptions): Promise<ProductMedia[]>;
  findByAssetId(assetId: MediaAssetId): Promise<ProductMedia[]>;
  findAssociation(productId: ProductId, assetId: MediaAssetId): Promise<ProductMedia | null>;
  ...
  setProductCoverImage(productId: ProductId, assetId: MediaAssetId): Promise<void>;
  removeCoverImageFlag(productId: ProductId): Promise<void>;
  updateCoverImage(productId: ProductId, newAssetId: MediaAssetId): Promise<void>;

  // Position and ordering management
  reorderProductMedia(productId: ProductId, mediaOrdering: Array<{ assetId: MediaAssetId; position: number }>): Promise<void>;
  moveMediaPosition(productId: ProductId, assetId: MediaAssetId, newPosition: number): Promise<void>;
  getNextPosition(productId: ProductId): Promise<number>;
  compactPositions(productId: ProductId): Promise<void>;

  // Bulk operations
  setProductMedia(productId: ProductId, mediaData: Array<{ assetId: MediaAssetId; position?: number; isCover?: boolean }>): Promise<void>;
  duplicateProductMedia(sourceProductId: ProductId, targetProductId: ProductId): Promise<void>;
  ...
  // Validation methods
  isMediaAssociatedWithProduct(productId: ProductId, assetId: MediaAssetId): Promise<boolean>;
