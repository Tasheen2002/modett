...
export class ProductMediaController {
  ...
  async getProductMedia(request: FastifyRequest<{ Params: { productId: string }; Querystring: ProductMediaQueryParams }>, reply: FastifyReply) {
    try {
      ...
      const productMedia = await this.productMediaManagementService.getProductMedia(productId, options);
  ...
  async addMediaToProduct(request: FastifyRequest<{ Params: { productId: string }; Body: AddMediaToProductRequest }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
  ...
  async removeMediaFromProduct(request: FastifyRequest<{ Params: { productId: string; assetId: string } }>, reply: FastifyReply) {
    try {
      ...
      await this.productMediaManagementService.removeMediaFromProduct(productId, assetId);
  ...
  async removeAllProductMedia(request: FastifyRequest<{ Params: { productId: string } }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
  ...
  async setProductCoverImage(request: FastifyRequest<{ Params: { productId: string }; Body: SetProductCoverImageRequest }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      await this.productMediaManagementService.setProductCoverImage(productId, assetId);
  ...
  async removeCoverImage(request: FastifyRequest<{ Params: { productId: string } }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
  ...
  async reorderProductMedia(request: FastifyRequest<{ Params: { productId: string }; Body: ReorderProductMediaRequest }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      for (const item of reorderData) {
        ...
        if (typeof item.position !== 'number' || item.position < 1) {
          return reply.code(400).send({
            success: false,
            error: 'Bad Request',
            message: 'Each reorder item must have a valid position (positive number)'
          });
        }
      }

      await this.productMediaManagementService.reorderProductMedia(productId, reorderData);
  ...
  async moveMediaPosition(request: FastifyRequest<{ Params: { productId: string }; Body: MoveMediaPositionRequest }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      await this.productMediaManagementService.moveMediaPosition(productId, assetId, newPosition);
      ...
      if (error instanceof Error && error.message.includes('Position must be greater than 0')) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: error.message
  ...
  async setProductMedia(request: FastifyRequest<{ Params: { productId: string }; Body: SetProductMediaRequest }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      for (const item of mediaData) {
        ...
        if (item.position !== undefined && (typeof item.position !== 'number' || item.position < 1)) {
          return reply.code(400).send({
            success: false,
            error: 'Bad Request',
      ...
      await this.productMediaManagementService.setProductMedia(productId, mediaData);
      ...
      if (error instanceof Error && error.message.includes('Only one media asset can be set as cover image')) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: error.message
  ...
  async duplicateProductMedia(request: FastifyRequest<{ Params: { sourceProductId: string; targetProductId: string } }>, reply: FastifyReply) {
    try {
      ...
      await this.productMediaManagementService.duplicateProductMedia(sourceProductId, targetProductId);
  ...
  async getProductsUsingAsset(request: FastifyRequest<{ Params: { assetId: string } }>, reply: FastifyReply) {
    try {
      ...
      const productIds = await this.productMediaManagementService.getProductsUsingAsset(assetId);
  ...
  async getAssetUsageCount(request: FastifyRequest<{ Params: { assetId: string } }>, reply: FastifyReply) {
    try {
      ...
      const usageCount = await this.productMediaManagementService.getAssetUsageCount(assetId);
  ...
  async compactProductMediaPositions(request: FastifyRequest<{ Params: { productId: string } }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      await this.productMediaManagementService.compactProductMediaPositions(productId);
  ...
  async validateProductMedia(request: FastifyRequest<{ Params: { productId: string } }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      const validation = await this.productMediaManagementService.validateProductMedia(productId);
  ...
  async getProductMediaStatistics(request: FastifyRequest<{ Params: { productId: string } }>, reply: FastifyReply) {
    try {
      const { productId } = request.params;
      ...
      const statistics = await this.productMediaManagementService.getProductMediaStatistics(productId);
