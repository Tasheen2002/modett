...
import { SizeGuide, SizeGuideId, Region } from "../../../domain/entities/size-guide.entity";
...
export class SizeGuideRepository implements ISizeGuideRepository {
  ...
  async findAll(options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    ...
    return sizeGuides.map(sizeGuideData => SizeGuide.fromDatabaseRow({
      size_guide_id: sizeGuideData.id,
      title: sizeGuideData.title,
      body_html: sizeGuideData.bodyHtml,
      region: sizeGuideData.region as Region,
  ...
  async findByRegion(region: Region, options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    const {
      limit = 50,
      offset = 0,
      sortBy = 'title',
  ...
  async findByCategory(category: string, options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    const {
      limit = 50,
      offset = 0,
      sortBy = 'title',
  ...
  async findByRegionAndCategory(region: Region, category: string): Promise<SizeGuide | null> {
