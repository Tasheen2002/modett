...
export interface ISocialLoginRepository {
  ...
  findByProviderUserId(provider: SocialProvider, providerUserId: string): Promise<SocialLogin | null>;
  findByUserIdAndProvider(userId: UserId, provider: SocialProvider): Promise<SocialLogin | null>;
  findActiveByUserId(userId: UserId): Promise<SocialLogin[]>;
  findInactiveByUserId(userId: UserId): Promise<SocialLogin[]>;

  // Business operations
  existsById(id: string): Promise<boolean>;
  existsByProviderUserId(provider: SocialProvider, providerUserId: string): Promise<boolean>;
  ...
  findByProviderEmail(provider: SocialProvider, email: string): Promise<SocialLogin[]>;
  findDuplicateConnections(provider: SocialProvider, providerUserId: string): Promise<SocialLogin[]>;
  findOrphanedConnections(): Promise<SocialLogin[]>;

  // Analytics operations
  getSocialLoginStatsByProvider(): Promise<Array<{
    provider: string;
    total: number;
    active: number;
    inactive: number;
