...
export class VerificationTokenRepository implements IVerificationTokenRepository {
  ...
  async findByToken(token: string, type: VerificationType): Promise<VerificationToken | null> {
  ...
  async findByUserIdAndType(userId: string, type: VerificationType): Promise<VerificationToken | null> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_tokens
  ...
  async findActiveByUserIdAndType(userId: string, type: VerificationType): Promise<VerificationToken | null> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_tokens
  ...
  async deleteByUserIdAndType(userId: string, type: VerificationType): Promise<void> {
    await this.prisma.$executeRaw`
