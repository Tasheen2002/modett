...
import { VerificationRateLimit, VerificationType } from "../../../domain/entities/verification-rate-limit.entity";

export class VerificationRateLimitRepository implements IVerificationRateLimitRepository {
  ...
  async findByUserIdAndType(userId: string, type: VerificationType): Promise<VerificationRateLimit | null> {
  ...
  async findByEmailAndType(email: string, type: VerificationType): Promise<VerificationRateLimit | null> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_rate_limits
      WHERE email = ${email}
        AND type = ${type}::user_management.verification_type_enum
  ...
  async findByPhoneAndType(phone: string, type: VerificationType): Promise<VerificationRateLimit | null> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_rate_limits
      WHERE phone = ${phone}
        AND type = ${type}::user_management.verification_type_enum
