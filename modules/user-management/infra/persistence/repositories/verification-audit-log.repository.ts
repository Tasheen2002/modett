...
import { VerificationAuditLog, VerificationType, VerificationAction } from "../../../domain/entities/verification-audit-log.entity";

export class VerificationAuditLogRepository implements IVerificationAuditLogRepository {
  ...
  async findByUserId(userId: string, limit = 100): Promise<VerificationAuditLog[]> {
  ...
  async findByEmail(email: string, limit = 100): Promise<VerificationAuditLog[]> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_audit_log
      WHERE email = ${email}
      ORDER BY created_at DESC
  ...
  async findByTypeAndAction(type: VerificationType, action: VerificationAction, limit = 100): Promise<VerificationAuditLog[]> {
    const result = await this.prisma.$queryRaw<any[]>`
      SELECT * FROM user_management.verification_audit_log
