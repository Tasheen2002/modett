...
export class PaymentMethodRepository implements IPaymentMethodRepository {
  ...
  async findByUserId(userId: UserId): Promise<PaymentMethod[]> {
    ...
    return paymentMethods.map(data => PaymentMethod.fromDatabaseRow({
  ...
  async findByUserIdAndType(userId: UserId, type: PaymentMethodType): Promise<PaymentMethod[]> {
    const paymentMethods = await this.prisma.paymentMethod.findMany({
      where: {
  ...
  async findByUserIdAndLast4(userId: UserId, last4: string): Promise<PaymentMethod[]> {
    const paymentMethods = await this.prisma.paymentMethod.findMany({
      where: {
        userId: userId.getValue(),
        last4,
  ...
  async getPaymentMethodStatsByType(): Promise<Array<{ type: string; count: number }>> {
  ...
  async getUserPaymentMethodStats(userId: UserId): Promise<{
    ...
    const [total, byType, defaultPaymentMethod, expiredCount, expiringSoonCount] = await Promise.all([
