...
export class ListPaymentMethodsHandler implements IQueryHandler<ListPaymentMethodsQuery, CommandResult<PaymentMethodsListResult>> {
  constructor(
    private readonly paymentMethodService: PaymentMethodService
  ) {}

  async handle(query: ListPaymentMethodsQuery): Promise<CommandResult<PaymentMethodsListResult>> {
    try {
      // Validate query
      if (!query.userId) {
      ...
      const paymentMethodResults: PaymentMethodResult[] = paymentMethods.map(pm => ({
