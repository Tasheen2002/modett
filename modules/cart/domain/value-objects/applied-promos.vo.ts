...
export class AppliedPromos {
  ...
  private validatePromo(promo: PromoData, index: number): void {
    ...
    if (!["percentage", "fixed_amount", "free_shipping", "buy_x_get_y"].includes(promo.type)) {
      throw new Error(`Promo at index ${index} has invalid type: ${promo.type}`);
    }

    if (typeof promo.value !== "number" || promo.value < 0) {
      throw new Error(`Promo at index ${index} must have a non-negative numeric value`);
    }

    if (promo.type === "percentage" && promo.value > 100) {
      throw new Error(`Percentage promo at index ${index} cannot exceed 100%`);
    }

    if (!(promo.appliedAt instanceof Date)) {
      throw new Error(`Promo at index ${index} must have a valid appliedAt date`);
    }
  }
  ...
  static fromJSON(json: string): AppliedPromos {
    try {
      ...
      throw new Error(`Invalid promo JSON: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
}