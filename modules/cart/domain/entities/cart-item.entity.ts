...
export class CartItem {
  ...
  static create(data: CreateCartItemData): CartItem {
    ...
    if (data.isGift && (!data.giftMessage || data.giftMessage.trim().length === 0)) {
      throw new Error("Gift message is required for gift items");
    }
  ...
  markAsGift(giftMessage: string): void {
    if (!giftMessage || giftMessage.trim().length === 0) {
      throw new Error("Gift message is required when marking item as gift");
    }
    this.isGift = true;
  ...
  updateGiftMessage(giftMessage: string): void {
    if (!this.isGift) {
      throw new Error("Cannot update gift message for non-gift items");
    }
    if (!giftMessage || giftMessage.trim().length === 0) {
  ...
  getDiscountAmount(): number {
    ...
    const totalDiscount = Math.min(percentageDiscountAmount + fixedDiscount, subtotal);
