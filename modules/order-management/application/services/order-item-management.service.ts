...
import { ProductSnapshot, ProductSnapshotData } from "../../domain/value-objects/product-snapshot.vo";
...
export class OrderItemManagementService {
  ...
  async addMultipleOrderItems(items: CreateOrderItemData[]): Promise<OrderItem[]> {
  ...
  async getTotalQuantityByVariant(variantId: string): Promise<number> {
    ...
    return await this.orderItemRepository.getTotalQuantityByVariantId(variantId);
  }
