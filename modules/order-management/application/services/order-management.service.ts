...
import { LoyaltyService, TransactionReason } from "../../../loyalty";
...
export class OrderManagementService {
  ...
  async markShipmentDelivered(data: {
    ...
    if (order.getUserId() && this.loyaltyService) {
      try {
        await this.loyaltyService.earnPointsFromPurchase(
          order.getUserId()!,
          order.getTotals().getTotal(),
          order.getOrderId().getValue()
        );
      } catch (error) {
        // Log but don't fail the delivery if loyalty points fail
        console.error('Failed to award loyalty points for delivered order:', error);
      }
    }
