...
export class GiftCard {
  ...
  redeem(amount: Money): void {
    if (!this.canRedeem(amount)) {
      throw new Error(
        "Cannot redeem - card is inactive, expired, or has insufficient balance"
      );
    }
  ...
  refund(amount: Money): void {
    if (!this._status.isActive() && !this._status.isRedeemed()) {
      throw new Error("Cannot refund - card must be active or redeemed");
    }
  ...
  expire(): void {
    if (!this.isExpired()) {
      throw new Error("Gift card expiry date has not passed");
    }
